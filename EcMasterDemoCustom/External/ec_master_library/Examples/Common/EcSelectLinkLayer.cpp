...
EC_T_BOOL bGetNextWord = EC_TRUE;
    ...
    *ptcWord = GetNextWord(lpCmdLine, tcStorage);
    ...
    else if ((EC_NULL != *ptcWord) && ((0 == OsStricmp(*ptcWord, "rpi5_sbc")))) /**< \EC_REFBOARD_EMLLGEM Raspberry Pi 5 SBC*/
    {
        if (1 == pLinkParmsAdapter->linkParms.dwInstance)

        pLinkParmsAdapter->dwPhyAddr           = 1;                       /**< PHY address */
        pLinkParmsAdapter->eGemType            = eGemType_BCM2712;        /**< System on Chip type */
        pLinkParmsAdapter->eRxSource           = eGemRxSource_MIO;        /**< Source of RX clock, control and data signals */
        pLinkParmsAdapter->dwRxInterrupt       = 0; //1                   /**< Receive interrupt number (IRQ) */
        pLinkParmsAdapter->bUseDmaBuffers      = EC_TRUE;                 /**< Use buffers from DMA (EC_TRUE) or from heap for receive. AllocSend is not supported, when EC_FALSE. */
        pLinkParmsAdapter->dwTxDmaDesCnt       = 64;                      /**< Transmit DMA descriptor buffer count. Must be a power of 2, maximum 256 */
        pLinkParmsAdapter->dwRxDmaDesCnt       = 64;                      /**< Receive DMA descriptor buffer count. Must be a power of 2, maximum 256 */
        pLinkParmsAdapter->bNoPhyAccess        = EC_TRUE;                /**< EC_FALSE: Link layer should initialize PHY and read link status (connected/disconnected). EC_TRUE: Client is responsible of PHY initialization and clock initialization */
        // pLinkParmsAdapter->linkParms           = 0;                       /**< Common link parameters. Signature must be set to EC_LINK_PARMS_SIGNATURE_GEM */
        pLinkParmsAdapter->ePhyInterface       = ePHY_RGMII;              /**< PHY connection type */
        pLinkParmsAdapter->bUseGmiiToRgmiiConv = EC_FALSE;                /**< Use XILINX GMIITORGMII Converter (EC_TRUE) */
        pLinkParmsAdapter->dwConvPhyAddr       = 0;                       /**< PHY address used to communicate with converter. In Linux doc it named "reg" */
        pLinkParmsAdapter->bNoPinMuxing        = EC_FALSE;                /**< No clock configuration and pin muxing */
        pLinkParmsAdapter->eClkDivType         = eGemClkDivType_default;  /**< Change Ref Clock settings */
        pLinkParmsAdapter->bNotUseCacheSync    = EC_TRUE;                 /**< Default use of CacheSync EC_FALSE, Don't call CacheSync on older systems EC_TRUE */
    }
    ...
    EC_T_LINK_PARMS** ppLinkParms)
{
    ...
    *ptcWord = GetNextWord(lpCmdLine, tcStorage);
    ...
    else if (0 == OsStricmp(*ptcWord, "rzg2l")) /**< \EC_REFBOARD_EMLLSHETH MYIR Remi Pi (Renesas RZ/G2L) */
    {
        pLinkParmsAdapter->eType = eSHEth_RZG2L;
        pLinkParmsAdapter->dwBaseAddr = 0x11C20000;
        pLinkParmsAdapter->byPhyAddr = 0;
        pLinkParmsAdapter->dwTxDmaDesCnt = 32;
        pLinkParmsAdapter->dwRxDmaDesCnt = 32;
    }
...
EC_T_VOID ShowSyntaxLinkLayer(EC_T_VOID)
{
    ...
    EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "     if custom        DW3504Type: allwinnert536 | cycloneV | intel_atom | lces1 | rk3328 | rk3399 | rk3562 | rk3568 | rk3576 | rk3588s  | semidrive-d9 | stm32mp157a-dk1 | stm32mp135\n"));
    ...
    EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "     Instance        Device instance (1=first), e.g. 1\n"));
    EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "     Mode            Interrupt (0) or Polling (1) mode\n"));
    EcLogMsg(EC_LOG_LEVEL_ERROR, (pEcLogContext, EC_LOG_LEVEL_ERROR, "     RefBoard        a800eva | rzg1e | rx72n | rzg2l \n"));
#endif
#if (defined INCLUDE_EMLLSIMULATOR)
