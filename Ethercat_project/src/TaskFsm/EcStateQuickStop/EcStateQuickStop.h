#ifndef EC_STATE_QUICK_STOP_H
#define EC_STATE_QUICK_STOP_H

#include "EcTaskStateBase.h"
#include "SubroutineBase.h"

class EcStateQuickStop : public EcTaskStateBase
{
public:
    EcStateQuickStop(uint32_t taskId ,const std::string& taskName, const std::string &taskPath);
    ~EcStateQuickStop();
    virtual uint32_t addSubroutine() override;
};

namespace EcStateQuickStopNs
{
