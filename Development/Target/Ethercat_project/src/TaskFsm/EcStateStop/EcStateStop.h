#ifndef EC_STATE_STOP_H
#define EC_STATE_STOP_H

#include "EcTaskStateBase.h"
#include "SubroutineBase.h"

class EcStateStop : public EcTaskStateBase
{
public:
    EcStateStop(uint32_t taskId ,const std::string& taskName, const std::string &taskPath);
    ~EcStateStop();
    virtual uint32_t addSubroutine() override;
};

namespace EcStateStopNs
{
