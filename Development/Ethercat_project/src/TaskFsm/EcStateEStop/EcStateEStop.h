#ifndef EC_STATE_E_STOP_H
#define EC_STATE_E_STOP_H

#include "EcTaskStateBase.h"
#include "SubroutineBase.h"

class EcStateEStop : public EcTaskStateBase
{
public:
    EcStateEStop(uint32_t taskId ,const std::string& taskName, const std::string &taskPath);
    ~EcStateEStop();
    virtual uint32_t addSubroutine() override;
};

namespace EcStateEStopNs
{
